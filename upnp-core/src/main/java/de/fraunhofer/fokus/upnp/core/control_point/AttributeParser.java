...
/**
 * This class is used to parse attribute lists returned by the AttributeService.
 * 
 * @author Alexander Koenig
 * 
 * 
 */
public class AttributeParser extends SAXTemplateHandler
{
  ...
  public void processContentElement(String content)
  {
    if (getTagCount() == 3)
    {
      ...
      if (getCurrentTag().equalsIgnoreCase("value"))
      {
  ...
  {
    if (getTagCount() == 2 && currentName != null && currentName.length() > 0 && currentValue != null)
    {
      attributeTable.put(currentName, currentValue);
    }
  }
